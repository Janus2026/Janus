...
int AscendDirectTransport::allocateLocalSegmentID() {
    ...
    uint64_t device_phy_id = static_cast<uint64_t>(device_logic_id_);
    char *phy_id_str = std::getenv("NPU_PHY_ID");
    if (phy_id_str) {
        device_phy_id = static_cast<uint64_t>(std::atoi(phy_id_str));
        LOG(INFO) << "Using NPU_PHY_ID from env: " << device_phy_id;
    }
    ...
    if (fin.is_open()) {
        std::string line;
        std::string key_prefix = "address_" + std::to_string(device_phy_id) + "=";
        while (std::getline(fin, line)) {
            if (line.rfind(key_prefix, 0) == 0) {
                device_ip = line.substr(key_prefix.size());
                device_ip.erase(0, device_ip.find_first_not_of(" \t"));
                device_ip.erase(device_ip.find_last_not_of(" \t\r\n") + 1);
                break;
            }
        }
        fin.close();
    } else {
        LOG(WARNING) << "Cannot open /etc/hccn.conf, deviceIp will not be set";
    }

    desc->rank_info.hostIp = host_ip;
    desc->rank_info.devicePhyId = device_phy_id;
    desc->rank_info.deviceLogicId = static_cast<uint64_t>(device_logic_id_);
    if (!device_ip.empty()) {
        desc->rank_info.deviceIp = device_ip;
    }
