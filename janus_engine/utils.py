...
def _run_dependencies_script_or_exit(script_path: str) -> None:
    shared_third_party = os.path.join(os.path.dirname(script_path), "third_party")
    dep_script = os.path.join(shared_third_party, "dependencies.sh")
    if not _run_shell_command(
        f"sh {dep_script}",
        cwd=script_path,
        passthrough_output=True,
    ):
        print(f"❌ Run shell command 'sh {dep_script}' failed!")
        _print_manual_check_commands([
            f"cd {script_path}",
            f"sh {dep_script}",
        ])
...
def _ensure_prebuild_dependencies_installed(script_path: str) -> None:
    ...
    if missing_dependencies:
        ...
        if missing_dependencies:
            ...
            shared_third_party = os.path.join(os.path.dirname(script_path), "third_party")
            dep_script = os.path.join(shared_third_party, "dependencies.sh")
            manual_commands = [f"cd {script_path}", f"sh {dep_script}"]
...
def _get_janus_ops_marker_path() -> str:
    ascend_home = os.getenv("ASCEND_HOME_PATH", "/usr/local/Ascend/ascend-toolkit/latest")
    opp_root = os.path.join(ascend_home, "opp")
    return os.path.join(opp_root, "vendors", "janus", ".janus_ops_git_head")


def _clear_janus_ops_cache_git_head(cache_path: str) -> bool:
    if not os.path.isfile(cache_path):
        return False

    cache_prefix = "JANUS_OPS_GIT_HEAD_CACHED:"
...
def _ensure_janus_ops_install_on_missing_marker() -> None:
    marker_path = _get_janus_ops_marker_path()
    if os.path.isfile(marker_path):
        return

    cmake_cache_path = _get_cmake_cache_path()
    if _clear_janus_ops_cache_git_head(cmake_cache_path):
        print("✅ Cleared JANUS_OPS_GIT_HEAD_CACHED from CMake cache to trigger janus_ops install.")
        return
...
def pre_build() -> None:
    ...
    _ensure_janus_ops_install_on_missing_marker()
