...
from utils import get_cpu_arch, get_device_type, pre_build, get_version, read_readme, get_cmake_dir, get_base_dir, get_python_version, get_torch_version
...
class ExtBuild(build_ext):
    user_options = build_ext.user_options + [
        ("base-dir=", None, "base directory of Janus project"),
    ...
    def build_extension(self, ext: CMakeExtension) -> None:
        ...
        cmake_args: list[str] = [
            ...
            f"-DJANUS_ATB_LAYERS_SOURCE_DIR={os.path.join(os.path.dirname(self.base_dir), 'third_party', 'janus_atb_layers')}",
    ...
    def build_cmake_targets(
        ...
        build_args += ["--target", ext.name, "janus"]
        subprocess.check_call(["cmake", "--build", ".", "--verbose"] + build_args, cwd=cmake_dir)

        os.makedirs(os.path.join(os.path.dirname(cmake_dir), "janus/core/server/"), exist_ok=True)
        shutil.copy(
            os.path.join(extdir, product),
            os.path.join(os.path.dirname(cmake_dir), "janus/core/server/"),
        )
...
class ExtBuildSingleTest(ExtBuild):
    ...
    def build_cmake_targets(
        ...
        possible_paths: list[str] = [
            ...
            os.path.join(cmake_dir, "janus", "core", self.test_name),
        ]
...
class BuildDistWheel(bdist_wheel):
    ...
    def run(self) -> None:
        ...
        if len(ext_path) == 0:
            ...
            exit(1)
        tmp_path = os.path.join(ext_path, 'janus')
...
def parse_arguments() -> dict[str, Any]:
    parser = argparse.ArgumentParser(
        description='Setup helper for building janus',
...
if __name__ == "__main__":
    ...
    print(f"🚀 Build janus with CPU arch: {arch} and target device: {device}")
    ...
    setup(
        ...
        license="Apache 2.0",
        author="Janus Team",
        author_email="Janus_2026@outlook.com",
        ...
        ext_modules=[CMakeExtension("janus", "janus/")],
        cmdclass={"build_ext": ExtBuild,
                  "test": test_cmd,
                  'bdist_wheel': BuildDistWheel},
        options=options,
        zip_safe=False,
        py_modules=["janus/launch_janus", "janus/__init__",
                    "janus/pybind/llm", "janus/pybind/vlm",
                    "janus/pybind/embedding", "janus/pybind/util",
                    "janus/pybind/args", "janus/pybind/params",
                    "janus/pybind/errors", "janus/pybind/mm_utils"],
        entry_points={
            'console_scripts': [
                'janus = janus.launch_janus:launch_janus'
            ],
        },
