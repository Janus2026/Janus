...
LOCAL_TARGET_DIR="${LOCAL_INSTALL_DIR}/janus"

HEADERS=("${SCRIPT_DIR}/llm.h" "${SCRIPT_DIR}/macros.h" "${SCRIPT_DIR}/types.h")
SO_FILES=(
    "${SCRIPT_DIR}/../../build/lib.linux-aarch64-cpython-311/janus/libjanus.so"
    "/usr/local/Ascend/ascend-toolkit/8.2.RC1/opp/vendors/janus/op_api/lib/libcust_opapi.so"
)
...
extract_to_local() {
    ...
    if [ -d "${LOCAL_TARGET_DIR}" ]; then
        rm -rf "${LOCAL_TARGET_DIR}" || error_exit "rm old janus directory failed"
    fi
