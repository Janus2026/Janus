...
import janus_export
from janus_export import (LLMMaster, VLMMaster, Options, RequestOutput,
                         RequestParams)
...
def _infer_model_backend(model_path: str) -> str:
    ...
    get_backend = getattr(janus_export, "get_model_backend", None)
    if not callable(get_backend):
        raise ValueError(
            "janus_export.get_model_backend is not available. "
            "Please rebuild janus_export or explicitly specify backend."
        )
    try:
...
class LLM:
    ...
    def sample(
        ...
        if len(params_list) > 1:
            # sample() mutates each RequestParams in-place (e.g. request_id/sample_slots).
            # Reusing the same object across concurrent batches causes overwrites.
