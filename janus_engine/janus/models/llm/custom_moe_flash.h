...
class CustomMoeFlashForCausalLMImpl
    : public LlmForCausalLMImplBase<DeepseekV2Model> {
 public:
  CustomMoeFlashForCausalLMImpl(const ModelContext& context)
    ...
    CHECK(!FLAGS_enable_prefix_cache)
        << "CustomMoeFlash have not supported "
...
TORCH_MODULE(CustomMoeFlashForCausalLM);

// register the causal model
REGISTER_CAUSAL_MODEL(custom_moe_flash, CustomMoeFlashForCausalLM);
// register the model args
// example config:
// https://huggingface.co/deepseek-ai/DeepSeek-V3/blob/main/config.json
REGISTER_MODEL_ARGS(custom_moe_flash, [&] {
  LOAD_ARG_OR(model_type, "model_type", "custom_moe_flash");
