...
#include "glm4v.h"
#include "models/llm/npu/custom_qwen3_npu.h"
...
namespace janus {
class CustomQwen3NpuVlmForConditionalGenerationImpl : public torch::nn::Module {
 public:
  CustomQwen3NpuVlmForConditionalGenerationImpl(const ModelContext& context)
      : model_args_(context.get_model_args()),
        options_(context.get_tensor_options()) {
    visual_ = register_module("visual", Glm4VisionTransformer(context));

    language_model_ =
        register_module("language_model", CustomQwen3NpuForCausalLM(context));
  }
 ...
 private:
  ...
  CustomQwen3NpuForCausalLM language_model_{nullptr};
};
TORCH_MODULE(CustomQwen3NpuVlmForConditionalGeneration);

REGISTER_INPUT_PROCESSOR(custom_qwen3_npu_vlm, Qwen2_5_VLInputProcessor);
REGISTER_CAUSAL_VLM_MODEL(custom_qwen3_npu_vlm, CustomQwen3NpuVlmForConditionalGeneration);
REGISTER_IMAGE_PROCESSOR(custom_qwen3_npu_vlm, Qwen2VLImageProcessor);
// register the model args
REGISTER_MODEL_ARGS(custom_qwen3_npu_vlm, [&] {
  LOAD_ARG_OR(model_type, "model_type", "custom_qwen3_npu_vlm");
