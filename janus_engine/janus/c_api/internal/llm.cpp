...
JANUS_CAPI_EXPORT JANUS_LLM_Handler* janus_llm_create(void) {
  JANUS_LLM_Handler* handler = new JANUS_LLM_Handler();
...
JANUS_CAPI_EXPORT void janus_llm_init_options_default(
  ...
  *init_options = JANUS_INIT_LLM_OPTIONS_DEFAULT;
}

JANUS_CAPI_EXPORT bool janus_llm_initialize(
  ...
  try {
    ...
    janus::helper::set_init_options(
        janus::helper::BackendType::LLM, init_options, &janus_init_options);
    ...
    options.model_path(model_path)
        ...
        .enable_prefill_sp(janus_init_options.enable_prefill_sp)
    ...
    handler->master = std::make_unique<janus::LLMMaster>(options);
    handler->master->run();
...
JANUS_CAPI_EXPORT void janus_llm_request_params_default(
  ...
  *request_params = JANUS_LLM_REQUEST_PARAMS_DEFAULT;
}
...
JANUS_CAPI_EXPORT JANUS_Response* janus_llm_completions(
  ...
  return janus::helper::handle_inference_request(
      handler,
      janus::helper::InferenceType::LLM_COMPLETIONS,
      model_id,
      prompt,
      nullptr,
      timeout_ms,
...
JANUS_CAPI_EXPORT JANUS_Response* janus_llm_chat_completions(
  ...
  return janus::helper::handle_inference_request(
      handler,
      janus::helper::InferenceType::LLM_CHAT_COMPLETIONS,
      model_id,
      janus_messages,
      nullptr,
      timeout_ms,
