#ifndef JANUS_REC_API_H
#define JANUS_REC_API_H
 ...
 * The handle MUST be created via janus_rec_create() and destroyed via
 * janus_rec_destroy() to prevent memory/device resource leaks.
 */
typedef struct JANUS_REC_Handler JANUS_REC_Handler;
 ...
 * APIs.
 * @return Valid JANUS_REC_Handler* on success; NULL if memory allocation fails
 * @see janus_rec_destroy
 */
 ...
 * @see janus_rec_create
 */
JANUS_CAPI_EXPORT void janus_rec_destroy(JANUS_REC_Handler* handler);

/**
 * @brief Helper to initialize JANUS_InitOptions with REC default values
 * Copies the predefined JANUS_INIT_REC_OPTIONS_DEFAULT values into the target
 ...
 * @see JANUS_INIT_REC_OPTIONS_DEFAULT, janus_rec_initialize
 */
 ...
 * batch size)
 * @see janus_rec_init_options_default, JANUS_INIT_REC_OPTIONS_DEFAULT,
 * janus_rec_create
 */
JANUS_CAPI_EXPORT bool janus_rec_initialize(JANUS_REC_Handler* handler,
                                          const char* model_path,
                                          const char* devices,
                                          const JANUS_InitOptions* init_options);

/**
 * @brief Helper to initialize JANUS_RequestParams with REC default values
 * Copies the predefined JANUS_REC_REQUEST_PARAMS_DEFAULT values into the target
 * request_params struct.
 * @param request_params Pointer to JANUS_RequestParams to initialize (NULL =
 * no-op)
 * @see JANUS_REC_REQUEST_PARAMS_DEFAULT, janus_rec_text_completions,
 * janus_rec_token_completions, janus_rec_chat_completions
 */
 ...
 * - kNotInitialized: Handler not initialized with janus_rec_initialize()
 ...
 * may be available)
 * @warning Mandatory: Call janus_rec_free_response() to release response memory
 * @see janus_rec_request_params_default, JANUS_REC_REQUEST_PARAMS_DEFAULT,
 * janus_rec_free_response
 */
 ...
 *
 * @param handler Valid, initialized REC inference instance handle (must not be
 * NULL) Created via janus_rec_create() and initialized via janus_rec_initialize()
 * @param model_id Null-terminated string of the loaded REC model ID (must match
 *                the model_path used in janus_rec_initialize())
 ...
 *
 * @par Response Status Codes (JANUS_StatusCode)
 ...
 * janus_rec_token_completions to support multi-modal recommendation scenarios
 ...
 * required)
 * @param mm_data Pointer to multi-modal data container (JANUS_MM_Data) (NULL =
 * no multimodal augmentation) Used to replace/augment information for specific
 * tokens in token_ids (via JANUS_MM_TokenPos) Supports
 * image/audio/video/embedding modalities (see JANUS_MM_Type) Must be valid
 * (mm_data->type_mask != JANUS_MM_TYPE_NONE) if non-NULL, and token positions in
 * mm_data must be within [0, token_size-1] (out-of-range positions trigger
 * kInvalidRequest)
 * @param timeout_ms Timeout in milliseconds (0 = no timeout, wait indefinitely)
 * @param request_params Generation parameters (NULL = use REC defaults)
 *                       See JANUS_RequestParams for configurable options (e.g.,
 * top_k, top_p)
 * @return Pointer to JANUS_Response on success; NULL ONLY if memory allocation
 ...
 *     - mm_data->type_mask = JANUS_MM_TYPE_NONE (empty multimodal data)
 *     - token positions in mm_data (JANUS_MM_TokenPos) are out of [0,
 * token_size-1] range
 ...
 * JANUS_MM_TokenPos.offset/length):
 ...
 * janus_rec_token_completions (text-only inference)
 *       6. Multimodal data must be aligned with token positions (offset +
 * length ≤ token_size)
 * @see janus_rec_token_completions, janus_rec_request_params_default,
 *      JANUS_REC_REQUEST_PARAMS_DEFAULT, janus_rec_free_response, JANUS_MM_Data,
 * JANUS_MM_TokenPos
 */
 ...
 * @param messages Array of JANUS_ChatMessage structs (recommendation-focused
 ...
 * JANUS_Response Releases all heap memory used by the REC response struct
 ...
 * @see janus_rec_text_completions, janus_rec_token_completions,
 * janus_rec_chat_completions
 */
