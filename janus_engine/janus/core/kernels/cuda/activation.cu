...
#define JANUS_LDG(arg) __ldg(arg)
...
__global__ void act_and_mul_kernel(
  ...
  if (aligned && d >= VEC_SIZE) {
    ...
    for (int i = threadIdx.x; i < num_vecs; i += blockDim.x) {
      int4 x = JANUS_LDG(&x_vec[i]), y = JANUS_LDG(&y_vec[i]), r;
...
#pragma unroll
    ...
    for (int i = vec_end + threadIdx.x; i < d; i += blockDim.x) {
      out_ptr[i] = compute<scalar_t, ACT_FN, act_first>(JANUS_LDG(&x_ptr[i]),
                                                        JANUS_LDG(&y_ptr[i]));
    }
  } else {
    // Scalar fallback for unaligned data or small d
    for (int64_t idx = threadIdx.x; idx < d; idx += blockDim.x) {
      const scalar_t x = JANUS_LDG(&x_ptr[idx]);
      const scalar_t y = JANUS_LDG(&y_ptr[idx]);
