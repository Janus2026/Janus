 ...
 *
 * Build:
 *   In janus project, build with cmake and run:
...
struct BenchmarkResult {
  ...
  int64_t n;
  int64_t k;
  janus::kernel::cuda::SM120DispatchKernel kernel;
...
BenchmarkResult RunBenchmark(int64_t m,
  ...
  if (num_sms == 0) {
    num_sms = static_cast<int>(
        janus::kernel::cuda::get_sm120_num_sms_for_device(device.index()));
  }

  auto dispatch = janus::kernel::cuda::select_sm120_dispatch(m, n, k, num_sms);
  result.kernel = dispatch.kernel;
  result.config_name =
      janus::kernel::cuda::get_sm120_dispatch_kernel_name(dispatch.kernel);
  result.wave_efficiency = janus::kernel::cuda::compute_sm120_wave_efficiency(
  ...
  for (int i = 0; i < warmup_iters; ++i) {
    janus::kernel::cuda::cutlass_scaled_mm(
  ...
  for (size_t i = 0; i < results.size(); ++i) {
    if (i > 0 && results[i].config_name != prev_config) {
      ...
      std::cout << "  M=" << results[i].m << ": " << prev_config << " ("
                << janus::kernel::cuda::get_sm120_dispatch_tile_shape_desc(
                       results[i - 1].kernel)
  ...
  for (const auto& [shape, results] : all_results) {
    ...
    for (const auto& [config_name, config_res] : config_results) {
      ...
      std::string tile_desc =
          janus::kernel::cuda::get_sm120_dispatch_tile_shape_desc(kernel);
      bool swap_ab = janus::kernel::cuda::is_sm120_swap_ab_kernel(kernel);
...
void GenerateRecommendations(
  ...
  for (const auto& [shape, results] : all_results) {
    ...
    for (const auto& r : results) {
      if (janus::kernel::cuda::is_sm120_swap_ab_kernel(r.kernel)) {
  ...
  for (const auto& [shape, results] : all_results) {
    ...
    for (const auto& r : results) {
      file << r.m << "," << r.n << "," << r.k << "," << r.config_name << ","
           << janus::kernel::cuda::get_sm120_dispatch_tile_shape_desc(r.kernel)
           << ","
           << (janus::kernel::cuda::is_sm120_swap_ab_kernel(r.kernel) ? "true"
                                                                     : "false")
...
void SaveAnalysisToJSON(
  ...
  for (const auto& kernel : janus::kernel::cuda::kSM120DispatchKernels) {
    if (!first_config) file << ",\n";
    first_config = false;
    const char* config_name =
        janus::kernel::cuda::get_sm120_dispatch_kernel_name(kernel);
    ...
    file << "      \"swap_ab\": "
         << (janus::kernel::cuda::is_sm120_swap_ab_kernel(kernel) ? "true"
                                                                 : "false")
         << "\n";
    file << "    }";
  }
...
int main(int argc, char* argv[]) {
  ...
  int compute_capability = janus::kernel::cuda::get_sm_version_num();
