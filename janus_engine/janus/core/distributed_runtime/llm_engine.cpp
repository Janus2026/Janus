...
janus::XTensorKvPageLayout build_engine_xtensor_kv_layout(
    const janus::ModelArgs& args,
    int32_t block_size,
    int64_t n_local_kv_heads,
    int64_t head_dim,
  ...
  return janus::build_xtensor_kv_layout(
...
void LLMEngine::process_group_test() {
  ...
  if (worker_clients_num_ > 1) {
    ...
    // JANUS_PROCESS_GROUP_ASYNC_TIMEOUT_SECONDS environment variable (default: 4
...
ForwardOutput LLMEngine::step(std::vector<Batch>& batch) {
  ...
  JANUS_DLOG(INFO) << "[DIAG-ENGINE] prepare_inputs starting";
  auto raw_forward_inputs = prepare_inputs(batch);
  JANUS_DLOG(INFO) << "[DIAG-ENGINE] prepare_inputs done, size="
  ...
  for (auto worker_rank = 0; worker_rank < worker_clients_num_; ++worker_rank) {
    ...
    JANUS_DLOG(INFO) << "[DIAG-ENGINE] dispatching step_async to worker_rank="
              << worker_rank;
    futures.emplace_back(
        worker_clients_[worker_rank]->step_async(raw_forward_inputs[dp_rank]));
  }

  JANUS_DLOG(INFO) << "[DIAG-ENGINE] all step_async dispatched, waiting collectAll";
  // wait for the all future to complete
  auto results = folly::collectAll(futures).get();
  JANUS_DLOG(INFO) << "[DIAG-ENGINE] collectAll completed";
