...
SpawnWorkerServer::SpawnWorkerServer(const std::string& master_node_addr,
  ...
  // TODO: pass whole janus::runtime::Options here from main process.
  janus::runtime::Options runner_options;
  ...
  const std::string device_type = janus::Device::type_str();
  const std::string device_str = device_type + ":" + std::to_string(device_idx);
  janus::Device device{torch::Device(device_str)};
  device.set_device();
