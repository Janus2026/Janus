...
bool run_inference(const std::string& input_embedding_path,
                   janus::VLMMaster& master,
                   const janus::RequestParams& sp) {
  torch::Tensor input_embedding = janus::load_tensor(input_embedding_path);
  std::string prompt =
      "<|im_start|>system\nYou are a helpful "
      "assistant.<|im_end|>\n<|im_start|>user\n(<image>./</"
      ...
      "start|>assistant\n";
  bool stream = false;
  janus::OutputCallback callback = [](janus::RequestOutput output) -> bool {
    if (output.finished) {
...
bool run_batch_inference(const std::string& input_embedding_path,
  ...
  torch::Tensor input_embedding = janus::load_tensor(input_embedding_path);
  janus::print_tensor(input_embedding, "input_embedding", -1, false, false);
  ...
  prompts.emplace_back(prompt);
  std::vector<janus::RequestParams> sps;
  sps.emplace_back(sp);
  bool stream = false;
  janus::BatchOutputCallback batch_callback =
      [](size_t index, janus::RequestOutput output) -> bool {
    if (output.finished) {
      std::cout << "output.outputs.size(): " << output.outputs.size()
                << std::endl;
      if (output.outputs.size() > 0) {
...
int main(int argc, char** argv) {
  ...
  janus::VLMMaster::Options option;
  ...
  std::cout << "begin init janus::VLMMaster==========" << std::endl;
  janus::VLMMaster master(option);

  // torch::Tensor image_tensor;
  // torch::Tensor image_tensor = torch::empty({});
  // torch::load(image_tensor, "/ktd/janus/image_tensor.pt");
  // torch::Tensor image_tensor = load_tensor("/ktd/janus/image_tensor.pt");
  // std::string prompt = "USER: <image>\nWhat are these?\nASSISTANT:";

  janus::RequestParams sp;
  sp.max_tokens = 1024;
  sp.temperature = 0;
