...
torch::Tensor FusedMoEImpl::select_experts(
  ...
  torch::Tensor expert_id;
  {
    ...
    std::tie(reduce_weight, expert_id) =
        janus::kernel::moe_active_topk(moe_active_topk_params);
  }
  ...
  if (is_smoothquant_) {
    janus::kernel::ScaledQuantizeParams scaled_quantize_params;
    ...
    std::tie(expand_hidden_states, hidden_states_scale) =
        janus::kernel::scaled_quantize(scaled_quantize_params);
    if (enable_all2all_communication) {
    ...
    janus::kernel::MoeExpandInputParams moe_expand_input_params;
    ...
    expand_hidden_states =
        janus::kernel::moe_expand_input(moe_expand_input_params);
    if (enable_all2all_communication) {
...
torch::Tensor FusedMoEImpl::forward_experts(const torch::Tensor& hidden_states,
  ...
  if (!stream_initialized_) {
    ...
    device_ = janus::Device(hidden_states.device());
  ...
  {
    janus::kernel::GroupGemmParams group_gemm_params;
    ...
    gemm1_out = janus::kernel::group_gemm(group_gemm_params);
  }
  ...
  // ensure the lifespan of these parameters via brace
  {
    ...
    gemm2_out = janus::kernel::group_gemm(group_gemm_params);
  }
  ...
  // ensure the lifespan of these parameters via brace
  {
    janus::kernel::MoeCombineResultParams moe_combine_result_params;
    ...
    final_hidden_states =
        janus::kernel::moe_combine_result(moe_combine_result_params);
  }
