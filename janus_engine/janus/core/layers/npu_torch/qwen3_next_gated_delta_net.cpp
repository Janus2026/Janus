...
torch::Tensor Qwen3NextGatedDeltaNetImpl::forward(
  ...
  if (attn_metadata.is_prefill) {
    ...
    janus::kernel::FusedGdnGatingParams gdn_params;
    ...
    std::tie(g, beta) = janus::kernel::fused_gdn_gating(gdn_params);
  }
