...
class DeepseekV2DecoderLayerTest : public ::testing::Test {
 protected:
  ...
  void sync_dev() const {
    janus::Device(options_.device()).synchronize_default_stream();
  }
  ...
  std::unique_ptr<janus::ProcessGroup> mock_process_group_;
...
TEST_F(DeepseekV2DecoderLayerTest, ForwardMixedDpMoEReturnsLocalSlice) {
  ...
  auto hidden_states =
      janus::layer::test::seeded_tensor("mixed_dp_moe.hidden_states",
...
TEST_P(DeepseekV2DecoderLayerParamTest,
  ...
  auto hidden_states = janus::layer::test::seeded_tensor(
      "hidden_states",
