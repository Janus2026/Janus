...
std::unique_ptr<janus::ProcessGroup> create_test_process_group(
    int32_t rank,
    int32_t world_size,
    int32_t port,
    const std::string& host,
    const torch::Device& device) {
  return janus::create_process_group(rank,
                                    world_size,
                                    world_size,
                                    port,
                                    false,
...
std::unique_ptr<janus::ProcessGroup> create_test_self_group(
    int32_t rank,
    int32_t world_size,
    int32_t port,
    const std::string& host,
    const torch::Device& device) {
  return janus::create_process_group(
      rank,
      world_size,
      /*rank_size=*/1,
...
void check_tensor_same_across_ranks(const torch::Tensor& tensor,
  ...
  torch::Tensor gathered =
      janus::parallel_state::gather(tensor, process_group, /*dim=*/0);
...
int32_t run_attention_test_child(int32_t rank,
                                 ...
                                 int64_t q_lora_rank) {
  try {
    const int32_t dev_count = janus::Device::device_count();
    ...
    janus::Device janus_device(device_index);
    janus_device.set_device();
    torch::Device device = janus_device.unwrap();
    auto process_group =
    ...
    janus_device.synchronize_default_stream();
