...
namespace janus::layer::v32_sp {
namespace {

class ScriptedAllGatherProcessGroup
    : public janus::layer::test::MockProcessGroup {
 public:
  ...
  c10::intrusive_ptr<c10d::Work> allgather_async(
    ...
    return janus::layer::test::make_completed_work();
  }
...
TEST(DeepseekV32SPUtilsTest, BuildContextRejectsMixedBatchForwardType) {
  ...
  janus::layer::test::MockProcessGroup process_group(torch::kCPU,
                                                    /*rank=*/0,
                                                    /*world_size=*/2);
...
TEST(DeepseekV32SPUtilsTest, GatherSupportsUnevenThreeDimensionalInputs) {
  ...
  torch::Tensor gathered = janus::parallel_state::gather(
...
TEST(DeepseekV32SPUtilsTest, LaunchAndFinishGatherMatchBlockingGather) {
  ...
  auto gather_ctx = janus::parallel_state::launch_gather(
      local_tensor, &process_group, std::vector<int32_t>{3, 2, 3, 1});
  torch::Tensor gathered = janus::parallel_state::finish_gather(gather_ctx);
