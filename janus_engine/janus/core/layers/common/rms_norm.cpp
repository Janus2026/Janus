...
std::tuple<torch::Tensor, std::optional<torch::Tensor>> RMSNormImpl::forward(
  ...
  janus::kernel::FusedLayerNormParams fused_layernorm_params;
  ...
  janus::kernel::fused_layernorm(fused_layernorm_params);
...
RMSNormImpl::forward_fp8(torch::Tensor& input,
  ...
  if (residual.has_value()) {
    ...
    janus::kernel::FusedAddRmsNormStaticFp8QuantParams params;
    params.input = input;
    ...
    auto [output, updated_residual] =
        janus::kernel::fused_add_rms_norm_static_fp8_quant(params);

    return std::make_tuple(output, updated_residual);
  } else {
    // RMSNorm + FP8 Quantization (no residual)
    janus::kernel::RmsNormStaticFp8QuantParams params;
    params.input = input;
    params.weight = weight_;
    params.scale = fp8_scale;
    params.epsilon = eps_;

    auto output = janus::kernel::rms_norm_static_fp8_quant(params);
