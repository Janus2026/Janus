...
void AddMatmulImpl::load_state_dict(const janus::StateDict& state_dict) {
  janus::weight::load_weight(state_dict, "weight", weight_, weight_is_loaded_);
  if (with_bias_) {
    janus::weight::load_weight(state_dict, "bias", bias_, bias_is_loaded_);
  }
}
...
void FusedAddMatmulImpl::load_state_dict(
    const janus::StateDict& state_dict,
