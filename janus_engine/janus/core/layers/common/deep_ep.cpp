...
void offload_exchange_info_allgather(
    janus::ProcessGroup* process_group,
...
DeepEPImpl::DeepEPImpl(int64_t dispatch_token_size,
  ...
  janus::kernel::MoeAll2AllCreateParams create_params;
  ...
  std::vector<torch::Tensor> created_tensors =
      janus::kernel::moe_all2all_create(create_params);
...
DeepEPImpl::~DeepEPImpl() {
  if (is_initialized_) {
    janus::kernel::MoeAll2AllDestroyParams destroy_params;
    destroy_params.handle = handle_;
    destroy_params.device = options_.device();
    janus::kernel::moe_all2all_destroy(destroy_params);
  }
}
...
void DeepEPImpl::dispatch(int64_t token_byte,
  ...
  janus::kernel::MoeAll2AllDispatchParams params;
  ...
  janus::kernel::moe_all2all_dispatch(params);
}
...
void DeepEPImpl::combine(int64_t token_byte,
                         ...
                         const std::optional<torch::Tensor>& send_token,
                         const std::optional<torch::Tensor>& recv_token) {
  janus::kernel::MoeAll2AllCombineParams params;
  params.handle = handle_;
  params.token_byte = token_byte;
  params.token_num = token_num;
...
void DeepEPImpl::dispatch_step(int64_t num_token_expand,
  ...
  torch::Tensor dispatch_send_layout =
      janus::kernel::moe_all2all_gen_send_layout(gen_send_layout_params);
...
DeepEPMetaResult DeepEPImpl::process_dispatch_result(
  ...
  janus::kernel::MoeAll2AllGenGatherIndexParams gen_gather_index_params;
  ...
  std::vector<torch::Tensor> gen_gather_index_output =
      janus::kernel::moe_all2all_gen_gather_index(gen_gather_index_params);
  ...
  janus::kernel::GatherSplitParams gather_split_params;
  ...
  janus::kernel::gather_split(gather_split_params);
