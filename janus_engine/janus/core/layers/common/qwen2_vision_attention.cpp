...
std::vector<torch::Tensor> Qwen2VisionAttentionImpl::split_qkv(
  ...
  torch::Tensor qkv_gathered =
      janus::parallel_state::all_gather_interleaved(qkv, tp_group_);
  ...
  if (tp_group_->world_size() > 1) {
    q = janus::parallel_state::scatter(q, tp_group_);
    k = janus::parallel_state::scatter(k, tp_group_);
    v = janus::parallel_state::scatter(v, tp_group_);
  }
...
torch::Tensor Qwen2VisionAttentionImpl::forward(
  ...
  janus::kernel::RotaryParams rotary_params;
  rotary_params.q = q;
  rotary_params.k = k;
  ...
  janus::kernel::apply_rotary(rotary_params);
  ...
  janus::kernel::mlu::batch_prefill(q,
                                   k,
                                   v,
                                   output,
                                   output_lse,
