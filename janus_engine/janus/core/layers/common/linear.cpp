...
void requantize_fp8_weight(torch::Tensor& weight,
  ...
  for (size_t idx = 0; idx < logical_widths.size(); ++idx) {
    ...
    janus::kernel::StaticScaledFp8QuantParams quant_params;
    quant_params.output = weight_quantized;
    quant_params.input = weight_fp16;
    quant_params.scale = scale_tensor;
    janus::kernel::static_scaled_fp8_quant(quant_params);
...
torch::Tensor fp8_linear_forward(
  ...
  if (input.dtype() == torch::kFloat8_e4m3fn) {
    ...
    janus::kernel::Fp8ScaledQuantizeParams quantize_params;
    ...
    std::tie(quantized_input, a_scale) =
        janus::kernel::fp8_scaled_quantize(quantize_params);
  }

  // FP8 scaled matmul
  janus::kernel::Fp8ScaledMatmulParams matmul_params;
  ...
  return janus::kernel::fp8_scaled_matmul(matmul_params);
}
...
torch::Tensor ColumnParallelLinearImpl::forward(torch::Tensor input) {
  ...
  if (quant_args_.quant_method() == kQuantMethodSmoothquant) {
    ...
    janus::kernel::ScaledQuantizeParams quantize_params;
    ...
    std::tie(quantized_input, input_scale) =
        janus::kernel::scaled_quantize(quantize_params);

    janus::kernel::ScaledMatmulParams matmul_params;
    matmul_params.a = quantized_input;
    ...
    output = janus::kernel::scaled_matmul(matmul_params);
  } else if (quant_args_.quant_method() == kQuantMethodFp8) {
    ...
    janus::kernel::MatmulParams matmul_params;
    matmul_params.a = input;
    matmul_params.b = weight_;
    matmul_params.bias = bias;
    output = janus::kernel::matmul(matmul_params);
  }

  if (world_size_ > 1 && gather_output_) {
    output = janus::parallel_state::gather(output, process_group_);
  }
  return output;
}
...
torch::Tensor QKVParallelLinearImpl::forward(torch::Tensor input) {
  ...
  if (world_size_ > 1 && gather_output_) {
    output = janus::parallel_state::gather(output, parallel_args_.tp_group_);
  }
  return output;
}
...
torch::Tensor RowParallelLinearImpl::forward(torch::Tensor input) {
  ...
  if (quant_args_.quant_method() == kQuantMethodSmoothquant) {
    ...
    if (!input_is_parallelized_) {
      input = janus::parallel_state::scatter(input, process_group_);
    }
  ...
  if (enable_result_reduction_ && world_size_ > 1) {
    output = janus::parallel_state::reduce(output, process_group_);
  }
  return output;
}
...
torch::Tensor ReplicatedLinearImpl::forward(torch::Tensor input) {
  ...
  auto output = janus::kernel::matmul(matmul_params);
  return output;
}
