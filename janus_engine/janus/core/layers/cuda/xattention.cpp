...
void XAttentionImpl::run_single_stage_decode(
  ...
  if (attn_metadata.enable_cuda_graph) {
    ...
    std::string backend = janus::kernel::cuda::determine_attention_backend(
        /*pos_encoding_mode=*/0,
  ...
  janus::kernel::cuda::batch_decode(attn_metadata.plan_info->uri,
...
void XAttentionImpl::run_two_stage_decode(
  ...
  janus::kernel::cuda::batch_prefill_non_causal(
  ...
  auto& xattention_workspace =
      janus::layer::xattention::XAttentionWorkspace::get_instance();
  ...
  janus::kernel::cuda::batch_decode(attn_metadata.unshared_plan_info->uri,
  ...
  janus::kernel::cuda::lse_combine(output,
                                  cache.shared_o,
                                  cache.shared_lse,
                                  cache.unshared_o,
                                  cache.unshared_lse);
...
void XAttentionImpl::prefill_forward(const AttentionMetadata& attn_metadata,
  ...
  if (attn_metadata.enable_cuda_graph) {
    ...
    flashinfer::update_prefill_plan_info(
        attn_metadata.plan_info,
        janus::kernel::cuda::determine_attention_backend(
            /*pos_encoding_mode=*/0,
            /*use_fp16_qk_reduction=*/false,
            /*use_custom_mask=*/false),
        attn_metadata,
  ...
  janus::kernel::cuda::prefill_reshape_and_cache(
  ...
  janus::kernel::cuda::batch_prefill_with_optional_piecewise_capture(
...
void XAttentionImpl::decoder_forward(const AttentionMetadata& attn_metadata,
  ...
  janus::kernel::cuda::decoder_reshape_and_cache(key,
                                                value,
