...
namespace {
bool mmvalue_to_proto(const janus::MMValue& cpp_value,
...
std::optional<janus::MMValue> proto_to_mmvalue(const proto::MMValue& pb_value) {
  if (pb_value.has_single_tensor()) {
    ...
    return janus::MMValue(torch_tensor);
    ...
    return janus::MMValue(torch_tensor_vec);
  } else {
...
bool mmdata_to_proto(const janus::MMBatchData& cpp_mmdata,
...
bool proto_to_mmdata(const proto::MMData& pb_mmdata,
  ...
  *cpp_mmdata = janus::MMBatchData(type, std::move(cpp_dict));
