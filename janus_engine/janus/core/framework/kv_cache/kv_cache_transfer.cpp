...
folly::SemiFuture<bool> KVCacheTransfer::push_kv_blocks_async(
  ...
  threadpool_.schedule([this,
    ...
    JANUS_DLOG(INFO) << "[DIAG] push_kv_blocks_async: merge_kv_blocks starting"
              << ", transfer_kv_infos.size=" << transfer_kv_infos.size();
    merge_kv_blocks(merged_kv_infos, transfer_kv_infos, parallel_args);
    JANUS_DLOG(INFO) << "[DIAG] push_kv_blocks_async: merge_kv_blocks done"
    ...
    if (!merged_kv_infos.empty()) {
      ...
      JANUS_DLOG(WARNING) << "[DIAG] merged_kv_infos is EMPTY, KV push skipped!";
    }
    JANUS_DLOG(INFO) << "[DIAG] push_kv_blocks_async completed, success=" << success;
    promise.setValue(success);
  });
  return future;
}
...
void KVCacheTransfer::merge_kv_blocks(
  ...
  for (auto& info : transfer_kv_infos) {
    ...
    JANUS_DLOG(INFO) << "[DIAG] merge_kv_blocks: dst_dp_rank=" << dst_dp_rank
    ...
    if (dst_dp_size <= 0) {
      JANUS_DLOG(ERROR) << "[DIAG] dst_dp_size is " << dst_dp_size
