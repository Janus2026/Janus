...
static inline void unmap_owned_pages(
    VirPtr vaddr,
    size_t page_size,
...
static inline void unmap_return_owned_pages_and_release_virtual_mem(
    VirPtr vaddr,
    size_t size,
    size_t page_size,
    std::unordered_map<page_id_t, std::unique_ptr<PhyPage>>& mapping) {
  unmap_owned_pages(vaddr, page_size, mapping);
  return_owned_pages_to_pool(mapping);
...
XTensor::~XTensor() {
  ...
  unmap_return_owned_pages_and_release_virtual_mem(
      vaddr_, size_, page_size_, mapping_);
}
