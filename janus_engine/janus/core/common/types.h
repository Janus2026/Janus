...
struct XTensorLayerOffsets {
  ...
  std::vector<uint64_t> index_offsets;  // Optional MLA index offsets
};

struct XTensorBlockBytes {
  uint64_t k_block_bytes = 0;
  uint64_t v_block_bytes = 0;
  uint64_t index_block_bytes = 0;

  [[nodiscard]] bool valid() const noexcept {
    return k_block_bytes > 0 && v_block_bytes > 0;
  }

  [[nodiscard]] bool has_index() const noexcept {
    return index_block_bytes > 0;
  }
};

struct XTensorOffsetsResponse {
...
class MasterStatus {
 public:
  ...
  constexpr MasterStatus(const janus::proto::MasterStatus& status) {
    switch (status) {
      case janus::proto::MasterStatus::WAKEUP:
        value_ = WAKEUP;
        break;
      case janus::proto::MasterStatus::LIGHT_SLEEP:
        value_ = LIGHT_SLEEP;
        break;
      case janus::proto::MasterStatus::DEEP_SLEEP:
        value_ = DEEP_SLEEP;
        break;
      default:
  ...
  constexpr janus::proto::MasterStatus to_proto() const {
    switch (value_) {
      case WAKEUP:
        return janus::proto::MasterStatus::WAKEUP;
      case LIGHT_SLEEP:
        return janus::proto::MasterStatus::LIGHT_SLEEP;
      case DEEP_SLEEP:
        return janus::proto::MasterStatus::DEEP_SLEEP;
      default:
        LOG(FATAL) << "Unsupported master status: " << value_;
        return janus::proto::MasterStatus::WAKEUP;  // unreachable, suppress
                                                   // warning
    }
  }
