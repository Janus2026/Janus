...
std::optional<ModelInputParams> CudaGraphPersistentParam::update(
  ...
  // Determine backend
  const std::string backend = janus::kernel::cuda::determine_attention_backend(
      /*pos_encoding_mode=*/0,
      /*use_fp16_qk_reduction=*/false,
      /*use_custom_mask=*/false);

  bool use_tensor_core =
      janus::kernel::cuda::should_use_tensor_core(dtype, n_heads, n_kv_heads);
...
bool CudaGraph::capture(CausalLM* model,
  ...
  if (FLAGS_enable_graph) {
    auto& capture_lock =
        ::janus::cuda::DeviceCaptureLock::get_instance().get_write_lock(
            device_index_);
...
ModelOutput CudaGraph::replay(const torch::Tensor& tokens,
  ...
  if (FLAGS_enable_graph) {
    auto& replay_lock =
        ::janus::cuda::DeviceCaptureLock::get_instance().get_read_lock(
            device_index_);
  ...
  if (is_piecewise_) {
    ...
    ::janus::kernel::cuda::AttentionReplayParams replay_params;
...
struct CudaGraphExecutorImpl::VmmPoolState {
  std::unique_ptr<janus::SharedVMMAllocator> allocator;
  std::unique_ptr<janus::VMMTorchAllocator> torch_allocator;
...
CudaGraphExecutorImpl::get_or_create_vmm_pool_state(uint32_t physical_pool_id) {
  ...
  if (!slot) {
    ...
    state->allocator = std::make_unique<janus::SharedVMMAllocator>();
    state->allocator->init(device_.index());
    state->torch_allocator =
        std::make_unique<janus::VMMTorchAllocator>(state->allocator.get());
    slot = std::move(state);
