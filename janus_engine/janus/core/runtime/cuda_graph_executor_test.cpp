...
torch::Device InitJanusCudaDeviceForTest(int32_t device_index = 0) {
  janus::Device janus_device(device_index);
  return janus_device.unwrap();
}
...
TEST(CudaGraphExecutorTest, BatchDecodeCaptureAndReplay) {
  ...
  const torch::Device device = InitJanusCudaDeviceForTest(/*device_index=*/0);
