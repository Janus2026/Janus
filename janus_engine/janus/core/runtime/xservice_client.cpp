...
namespace janus {
namespace {
static std::string ETCD_MASTER_SERVICE_KEY = "JANUS:SERVICE:MASTER";
static std::string ETCD_XSERVICES_KEY_PREFIX =
    "JANUS:SERVICE:";  // all janus_service registeration prefix
static std::unordered_map<janus_service::proto::InstanceType, std::string>
    ETCD_KEYS_PREFIX_MAP = {
        {janus_service::proto::InstanceType::DEFAULT, "JANUS:DEFAULT:"},
        {janus_service::proto::InstanceType::PREFILL, "JANUS:PREFILL:"},
        {janus_service::proto::InstanceType::DECODE, "JANUS:DECODE:"},
        {janus_service::proto::InstanceType::MIX, "JANUS:MIX:"},
};
...
std::string parse_instance_name(const std::string& name) {
  ...
  if (pos == std::string::npos) {
    ...
    return janus::net::get_local_ip_addr() + ":" + name;
  }
  return name;
}
...
bool XServiceClient::init(const std::string& etcd_addr,
  ...
  // Get and connect to all existing janus_service instances.
  ...
  // watch master janus_service change
  ...
  // watch all janus_service changes
...
void XServiceClient::register_instance(const InstanceInfo& instance_info) {
  ...
  if (InstanceRole(registered_info.type) == InstanceRole::DEFAULT) {
    key_prefix =
        ETCD_KEYS_PREFIX_MAP[janus_service::proto::InstanceType::DEFAULT];
  } else if (InstanceRole(registered_info.type) == InstanceRole::PREFILL) {
    key_prefix =
        ETCD_KEYS_PREFIX_MAP[janus_service::proto::InstanceType::PREFILL];
  } else if (InstanceRole(registered_info.type) == InstanceRole::DECODE) {
    key_prefix =
        ETCD_KEYS_PREFIX_MAP[janus_service::proto::InstanceType::DECODE];
  } else if (InstanceRole(registered_info.type) == InstanceRole::MIX) {
    key_prefix = ETCD_KEYS_PREFIX_MAP[janus_service::proto::InstanceType::MIX];
  } else {
...
InstanceInfo XServiceClient::get_instance_info(
  ...
  janus_service::proto::InstanceID req;
  janus_service::proto::InstanceMetaInfo resp;
  req.set_name(instance_name);

  std::string master_addr;
  if (!with_master_stub(
          [&](janus_service::proto::JanusRpcService_Stub* master_stub) {
  ...
  if (resp.type() == janus_service::proto::InstanceType::PREFILL) {
    result.type = "PREFILL";
  } else if (resp.type() == janus_service::proto::InstanceType::DECODE) {
    result.type = "DECODE";
  } else if (resp.type() == janus_service::proto::InstanceType::MIX) {
    result.type = "MIX";
  } else {
    result.type = "DEFAULT";
  }
...
void XServiceClient::heartbeat() {
  ...
  while (!exited_.load()) {
    ...
    brpc::Controller cntl;
    janus_service::proto::HeartbeatRequest req;
    ...
    janus_service::proto::Status resp;
    std::string master_addr;
    if (!with_master_stub(
            [&](janus_service::proto::JanusRpcService_Stub* master_stub) {
...
std::vector<std::string> XServiceClient::get_static_decode_list() {
  brpc::Controller cntl;
  janus_service::proto::InstanceID req;
  janus_service::proto::InstanceIDs resp;
  req.set_name(instance_name_);
...
std::vector<bool> XServiceClient::generations(
  ...
  // group requests by target janus_service
  ...
  // send requests to each janus_service in parallel
...
bool XServiceClient::connect_to_xservice(const std::string& xservice_addr) {
  ...
  xservice_stubs_[xservice_addr] =
      std::make_unique<janus_service::proto::JanusRpcService_Stub>(
...
janus_service::proto::JanusRpcService_Stub* XServiceClient::find_stub_locked(
