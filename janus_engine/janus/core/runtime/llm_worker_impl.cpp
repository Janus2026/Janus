...
LLMWorkerImpl::LLMWorkerImpl(const ParallelArgs& parallel_args,
  ...
  threadpool_.schedule([this]() mutable {
    ...
    ::janus::layer::flashinfer::FlashinferWorkspace::get_instance().initialize(
        device_);
  });
#endif
}
...
std::optional<ForwardOutput> LLMWorkerImpl::step_internal(
  ...
  JANUS_DLOG(INFO) << "[DIAG] LLMWorkerImpl::step() acquired lock"
  ...
  JANUS_DLOG(INFO) << "[DIAG] model_executor_->forward() starting";
  auto model_output = model_executor_->forward(
      input.token_ids, input.positions, kv_caches_, input.input_params);
  JANUS_DLOG(INFO) << "[DIAG] model_executor_->forward() completed";
  ...
  if (!enable_schedule_overlap() && !driver_ && !dp_driver_ &&
    ...
    if (options_.kv_cache_transfer_mode() == "PUSH" &&
        !input.transfer_kv_infos.empty()) {
      JANUS_DLOG(INFO) << "[DIAG] Waiting for KV push futures (path1)...";
      auto results =
          folly::collectAll(futures).within(std::chrono::seconds(60)).get();
      JANUS_DLOG(INFO) << "[DIAG] KV push futures completed (path1)";
  ...
  if (options_.kv_cache_transfer_mode() == "PUSH" &&
      !input.transfer_kv_infos.empty()) {
    JANUS_DLOG(INFO) << "[DIAG] Waiting for KV push futures (path2)...";
    auto results =
        folly::collectAll(futures).within(std::chrono::seconds(60)).get();
    JANUS_DLOG(INFO) << "[DIAG] KV push futures completed (path2)";
    for (const auto& result : results) {
      // TODO: Add error handling
      if (!result.value()) {
        LOG(ERROR) << "kv_cache_transfer_ failed";
