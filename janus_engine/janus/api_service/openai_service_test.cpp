// Testing steps:
// 1. start the janus server first
// 2. run the test with disabled cases enabled
// JANUS_TEST_BASE_URL=http://127.0.0.1:9977 JANUS_TEST_MODEL=Qwen3-8B
//     ./build/lib.linux-aarch64-cpython-311/janus/openai_service_test
...
struct TestConfig {
  ...
  static TestConfig get() {
    TestConfig config;
    const char* url_env = std::getenv("JANUS_TEST_BASE_URL");
    config.base_url = url_env ? url_env : "http://127.0.0.1:9977";

    const char* model_env = std::getenv("JANUS_TEST_MODEL");
    config.model = model_env ? model_env : "my_model";

    const char* key_env = std::getenv("JANUS_TEST_API_KEY");
    config.api_key = key_env ? key_env : "janus-test-123456";
    return config;
  }
};
