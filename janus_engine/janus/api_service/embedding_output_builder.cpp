...
bool TensorProtoBuilder::build_repeated_tensor(
  ...
  for (const auto& in_tensor : in_tensors) {
    ...
    janus::proto::Tensor* out_tensor = out_tensors.Add();
...
bool TensorProtoBuilder::build_tensor(const janus::proto::Tensor& in_tensor,
...
bool EmbeddingOutputBuilder::build_repeated_embedding_output(
    ...
    google::protobuf::RepeatedPtrField<janus::proto::Embedding>& out_embeddings,
    std::string& binary_payload) {
  for (const auto& in_embedding : in_embeddings) {
    janus::proto::Embedding* out_embedding = out_embeddings.Add();
...
bool EmbeddingOutputBuilder::build_embedding_output(
  ...
  for (const auto& [key, value] : in_embedding.metadata) {
    janus::proto::Tensor metadata_tensor;
...
bool EmbeddingOutputBuilder::build_embedding_output(
    const janus::proto::Embedding& in_embedding,
    std::string& binary_payload,
